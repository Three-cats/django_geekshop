...
class OrderCreate(CreateView, BaseClassContextMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            if basket_item:
                OrderFormSet = inlineformset_factory(
                    Order, OrderItem, form=OrderItemsForm, extra=basket_item.count())
...
def order_change_status(request, pk):
    order = Order.objects.get(pk=pk)
    status = order.status
    my_tuple = Order.ORDER_STATUS_CHOICES
    for i in my_tuple:
        if i[0] == status and i[0] != 'CNC':
            ind = my_tuple.index(i)
            order.status = my_tuple[ind + 1][0]
            order.save()
    return HttpResponseRedirect(reverse('orders:list'))
